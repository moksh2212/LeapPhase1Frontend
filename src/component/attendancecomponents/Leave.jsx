        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell><Typography variant="subtitle1">Reason Subject</Typography></TableCell>
              <TableCell><Typography variant="subtitle1">Leave From</Typography></TableCell>
              <TableCell><Typography variant="subtitle1">Leave Till</Typography></TableCell>
              <TableCell><Typography variant="subtitle1">Approval Status</Typography></TableCell>
          ...
          <TableBody>
            {requests.map((request) => (
              <TableRow key={request.leaveId} >
                ...
                {isPendingTab && (
                  <TableCell>
                    <Button variant="contained" color="primary" onClick={() => handleRowClick(request)}>
                      View Details
                    </Button>
                  </TableCell>
                )}
        ...
        {request && (
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <Typography variant="subtitle1">Employee ID: {request.talentId}</Typography>
              <Typography variant="subtitle1">Employee Name: {request.talentName}</Typography>  
              <Typography variant="subtitle1">Subject: {request.subject}</Typography>
              <Typography variant="subtitle1">Leave From: {request.startDate}</Typography>
              <Typography variant="subtitle1">Leave Till: {request.endDate}</Typography>
            </Grid>
            <Grid item xs={12} md={6}>
              <Typography variant="subtitle1" style={{ marginBottom: '8px' }}>Detailed Reason:</Typography>
              <DialogContentText style={{ whiteSpace: 'pre-line' }}>{request.description}</DialogContentText>
            </Grid>
          </Grid>
        )}
      </DialogContent>
...
const Leave = () => {
  ...
  const handleApproveRequest = async (leaveId) => {
    try {
      ...
      const updatedRequests = leaveRequests.map(request =>
        request.leaveId === leaveId ? { ...request, approvalStatus: 'Approved' } : request
      );
  ...
  const handleRejectRequest = async (leaveId) => {
    try {
      ...
      const updatedRequests = leaveRequests.map(request =>
        request.leaveId === leaveId ? { ...request, approvalStatus: 'Declined' } : request
      );
      setLeaveRequests(updatedRequests);
